...
void parallel_histogram(const int *data, int n, int *hist, int bins) {
    // TODO: Use #pragma omp parallel for to iterate over data.
    // TODO: Use #pragma omp critical to protect updates to hist.
    // Note: You may also try #pragma omp atomic or omp_lock_t to protect updates.
    for (int i = 0; i < bins; i++) {
        hist[i] = 0;
    }
