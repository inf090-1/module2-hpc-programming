# 5. Matrix Multiplication Optimizations
...
## Exercise
- `matmul_omp_simd.c`: Implement the OpenMP + SIMD version and compare against serial + BLAS.

## Compilation and Execution on the INF0090 Cluster

First, compile your code on the login node. You will need to link the BLAS library for the BLAS example (e.g., `-lopenblas`):
```bash
...
![Matmul Roofline](figures/matmul_roofline.png)

The benchmark produces `results.csv` on the cluster. To regenerate the roofline plot:
...
2. Copy `results.csv` back to your local machine:
   ```bash
   scp info090:~/02-openmp-basics/5-matmul/results.csv .
   ```
3. Run the roofline script locally (requires `numpy` + `matplotlib`):
   ```bash
   python3 roofline_matmul.py --results results.csv --out figures/matmul_roofline.png
   ```
