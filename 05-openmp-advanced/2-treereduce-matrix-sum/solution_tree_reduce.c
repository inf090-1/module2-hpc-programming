...
// TreeReduce: matrix sum using an OpenMP task-based binary reduction tree.
                                  ...
                                  int size)
{
    ...
    // Spawn two tasks for left/right halves.
    #pragma omp taskgroup
    {
        #pragma omp task firstprivate(left, mid, size) shared(mats, tmpL)
        {
            tree_reduce_sum_range(mats, left, mid, tmpL, size);
        }

        #pragma omp task firstprivate(mid, right, size) shared(mats, tmpR)
        {
            tree_reduce_sum_range(mats, mid, right, tmpR, size);
        }
    }

    // Combine results (use double to reduce rounding error).
...
int main(void) {
    ...
    // Solution: task-based tree reduction.
    double t_task0 = omp_get_wtime();
    #pragma omp parallel
    {
        #pragma omp single
        {
            tree_reduce_sum((const float* const*)mats, NUM_MATS, out_tasks, size);
        }
    }
    ...
    // Validate.
    double max_abs_err_tasks = 0.0;
    for (int i = 0; i < size; ++i) {
        double err = (double)out_tasks[i] - (double)out_ref[i];
        if (err < 0) err = -err;
        if (err > max_abs_err_tasks) max_abs_err_tasks = err;
    }
    ...
    const int ok = (max_abs_err_tasks <= eps);
    ...
    printf("[tree-reduce-matrix] solution_tasks_time=%.6f s\n", t_task);
    printf("[tree-reduce-matrix] max_abs_err_tasks=%g (eps=%g) | %s\n",
           max_abs_err_tasks, eps, ok ? "PASS" : "FAIL");
