...
```bash
mpirun -np 2 ./ping-pong-exercise
mpirun -np 2 ./ping-pong-solution --bufsize 10000
```
...
```bash
srun --partition=cpu --nodes=1 --ntasks=2 --mpi=pmix ./ping-pong-exercise
srun --partition=cpu --nodes=1 --ntasks=2 --mpi=pmix ./ping-pong-solution
```
